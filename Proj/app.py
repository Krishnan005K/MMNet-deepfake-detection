...
def video_artifact_heuristic(video_path):
    ...
    # normalize mean_blur to [0..1] where lower sharpness => higher fake score
...
def run_pipeline(video_path, model_path, out_audio="extracted_audio.wav", device_str=None, debug=False):
    ...
    print(f"Audio fake p : {fake_p:.4f}   (audio model)")
    print(f"Lip-sync p   : {lip_fake_prob:.4f} (1 - sync_score; 0=good sync)")
    print(f"Video art p  : {video_fake:.4f} (heuristic)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Full deepfake checker (audio model + lip-sync + heuristics).")
